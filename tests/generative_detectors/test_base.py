...
def test_content_analysis_invalid_detection_object(detection_base):
    """Test that content_analysis returns error if the chatcompletionresponse returns the wrong type."""
    base_instance = asyncio.run(detection_base)
    content_request = ContentsDetectionRequest(
        contents=["Where do I find geese?"]
    )
    with patch(
        "vllm_detector_adapter.generative_detectors.base.ChatCompletionDetectionBase.process_chat_completion_with_scores",
        return_value=("dummy_response", "dummy_scores", "dummy_detection_type", "dummy_metadata"),  # 4-tuple
    ), patch(
        "vllm_detector_adapter.generative_detectors.base.ChatCompletionDetectionBase.post_process_completion_results",
        # must accept exactly 4 arguments, so lambda w/ 4 params
        side_effect=lambda response, scores, detection_type, metadata: ("dummy_processed_response", "dummy_processed_scores", "dummy_processed_type", "dummy_processed_metadata"),
    ), patch(
        "vllm_detector_adapter.generative_detectors.base.ContentsDetectionResponseObject.from_chat_completion_response",
        return_value=("not", "a", "valid", "object"),  # bad type
    ):
        result = asyncio.run(base_instance.content_analysis(content_request))
